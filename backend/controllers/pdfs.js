...
const uploadpdf = async (req, res) => {
    try {
        ...
        const response = await axios.post('http://localhost:3001/en/getparameters', { text: parsedText });
        if (response.data.data === false) {
            return res.json({ data: false });
        }
        ...
        const analysisResponse = await axios.post('http://localhost:3001/en/analysis', {
            fileId: fileId,
